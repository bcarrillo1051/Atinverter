...
# **50/60Hz PWM Inversion Library Feature**
***

## 📋 Overview
This section aims to document the functionality of the Atinverter library’s PWM generation feature, used to synthesize a sinusoidal waveform using a timer-driven software control scheme. The PWM waveform is composed of high-frequency switching pulses generated by Timer1 interrupts, which step through a precomputed sine lookup table at a rate of 31,372 kHz. Simultaneously, Timer0 is configured to Phase Correct PWM mode and alternates the output between two hardware PWM pins to approximate a 50Hz or 60Hz AC signal. The software manages duty cycle updates and pin toggling using ISR logic, ultimately enabling fully software-defined sinusoidal PWM output from the ATMEGA328P.
...
## 🧮 50Hz and 60Hz Sinusoid Lookup Table
...
**Purpose:** To generate the alternating sinusoidal PWM signals on the digital pins 5 and 6 by iterating through a lookup table of precomputed PWM values. 

**Pseudocode:**
1. Select sinusoid lookup table based on frequency mode flag `is50Hz`
2. Determine the max index for lookup table based on the frequency mode flag
3. If the end of the lookup table has been reached and the digital pin flag `OK` is set LOW (indicates that digital pin 6 is currently active except for first `pwmISR()` iteration):
  - Reset lookup table index and toggle the digital pin flag to activate pin 5 and deactivate pin 6 in the same cycle
4. If the end of the lookup table has been reached and the digital pin flag `OK` is set HIGH (indicates that digital pin 5 is currently active except for first `pwmISR()` iteration):
  - Reset lookup table index and toggle to digital pin flag to activate pin 6 and deactivate pin 5 in the same cycle
5. Load PWM pulse value from lookup table
6. Increment the lookup table index `sin_i` to move to the next sample
7. If OK flag is off:
  - Set digital pin 5 (`OCR0B`) off
  - Set digital pin 6 (`OCR0A`) to output current PWM pulse
8. If OK flag is on:
  - Set digital pin 6 (`OCR0A`) off
  - Set digital pin 5 (`OCR0B`) to output current PWM pulse

**Implementation in `Atinverter.cpp`:**
```cpp
void Atinverter::pwmISR() {
  const int* sinPWM = is50Hz ? sin50HzPWM : sin60HzPWM;

  int maxIndex = is50Hz ? SIN_50HZ_PWM_SIZE : SIN_60HZ_PWM_SIZE;

  if (sin_i > maxIndex && OK == 0) {
    sin_i = 0;
    OK = 1;
  }
...
**Purpose:** Enables the Timer1 interrupts so that PWM generation resumes.

**Pseudocode:**
1. Disable global interrupts
2. Enable the Timer1 Compare Match A interrupt (`OCIE1A`)
...
**Purpose:** Disables the Timer1 interrupts to stop PWM generation without affecting other global interrupts.

**Pseudocode:**
1. Disable global interrupts
2. Disable the Timer1 Compare Match A interrupt interrupts (`OCIE1A`)
3. Re-enable global interrupts
