/*
  AtInverterE.cpp - Library for AtInverterE pin and function mapping
...
// sets up the pin mode for all AtInverterE pins
...
void AtInverterE::initializePWMTimer()
{
...
// example usage: AtInverterE.initializeInterruptTimer(1, &controlUpdate);

void AtInverterE::initializeInterruptTimer(int periodms, void interruptFunction())
{
...
void AtInverterE::setDutyCycle(int dutyCycle)
{
...
int AtInverterE::raw2mV(int raw)
{
  long rawL = (long)raw;
...
// assumes VCC = 5V; if not the case, will have to readVCC
unsigned int AtInverterE::raw2mVActual(int raw)
{
  long rawL = (long)raw;
...
// assumes VCC = 5V; if not the case, will have to readVCC
int AtInverterE::raw2mA(int raw)
{
  long rawL = (long)raw;
